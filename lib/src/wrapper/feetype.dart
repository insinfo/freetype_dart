...
class Feetype implements Finalizable {
  String dllPath = r'C:\MyProjectsDart\freetype\libfreetype-6.dll';
  ...
  static final Finalizer<Feetype> _finalizer =
      Finalizer((instance) => instance.free());

  Feetype({DynamicLibrary? dl, String? dllPath, this.allocator = calloc}) {
    if (dl == null) {
      this.dllPath = dllPath == null ? this.dllPath : dllPath;
  ...
  Face newFace(String path, {int faceIndex = 0}) {
    ...
    final pathP = path.asInt8(allocator: allocator);
...
class Bitmap {
  ...
  List<int> get buffer {
    ...
    if (bufferSize > 0) {
      return slot.ref.bitmap.buffer.asTypedList((pitch * rows).toInt());
    } else {
      return <int>[];
    }
  }
